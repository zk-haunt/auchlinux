...
class CavaDataParser:
    ...
    @staticmethod
    def get_default_bars(bar_chars, width):
        """Construct a default bar pattern where the 2nd and second-to-last bars have a small bump"""
        low_char = bar_chars[0]
        bump_char = bar_chars[1] if len(bar_chars) > 1 else low_char
        if not width or width < 3:
            return low_char * (width or len(bar_chars))
        chars = [low_char] * width
        chars[1] = bump_char
        chars[width - 2] = bump_char
        return "".join(chars)
                    ...
                    if proc and proc.stdout:
                        ...
                        if rlist:
                            ...
                            if line_stripped:
                                ...
                                if values and all(int(v) <= 2 for v in values):
                                    self.consecutive_zero_count += 1
                                    if self.consecutive_zero_count == self.zero_threshold:
                                        self._broadcast_data(b"STANDBY\n")
                                    elif self.consecutive_zero_count < self.zero_threshold:
            ...
            while True:
                ...
                if decoded_data.strip():
                    ...
                    while "\n" in buffer:
                        ...
                        if line.strip():
                            if line.strip() == "STANDBY":
                                if json_output:
                                    output = {
                                        "text": "",
                                        "tooltip": "Cava audio visualizer - standby",
                                    }
                                    print(json.dumps(output), flush=True)
                                else:
                                    print("", flush=True)
                                ...
                                is_silent = False
                                try:
                                    is_silent = all(int(x) <= 2 for x in line.strip().split(";") if x.isdigit())
                                except Exception:
                                    pass

                                if is_silent:
                                    formatted = CavaDataParser.get_default_bars(bar_chars, width)
                                else:
