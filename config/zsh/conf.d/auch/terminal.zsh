...
function _dedup_zsh_plugins {
    ...
    # Load Plugins
    auch_plugins=(git zsh-256color zsh-autosuggestions zsh-syntax-highlighting)
    plugins+=("${plugins[@]}" "${auch_plugins[@]}")
    # Deduplicate plugins
    plugins=("${plugins[@]}")
    plugins=($(printf "%s\n" "${plugins[@]}" | sort -u))
...
function _load_deferred_plugin_system_by_auch() {

    # Exit early if AUCH_ZSH_DEFER is not set to 1
    if [[ "${AUCH_ZSH_DEFER}" != "1" ]]; then
        unset -f _load_deferred_plugin_system_by_auch
        return
    fi
...
function _load_compinit() {
    ...
    if [[ -n ${ZDOTDIR}/.zcompdump(#qN.mh+${AUCH_ZSH_COMPINIT_CHECK:-1}) ]]; then
        compinit
    else
        compinit -C
    fi
...
function _load_prompt() {
    ...
    if ! source ${ZDOTDIR}/prompt.zsh >/dev/null 2>&1; then
        [[ -f $ZDOTDIR/conf.d/auch/prompt.zsh ]] && source $ZDOTDIR/conf.d/auch/prompt.zsh
    fi
}
...
AUCH_ZSH_DEFER="1"      #Unset this variable in $ZDOTDIR/user.zsh to disable deferred Zsh loading.
AUCH_ZSH_PROMPT="1"     #Unset this variable in $ZDOTDIR/user.zsh to disable prompt customization.
AUCH_ZSH_NO_PLUGINS="0" #Set this variable to "1" in $ZDOTDIR/user.zsh to disable Zsh plugin loading.
...
if [[ -f $HOME/.auch.zshrc ]]; then
    source $HOME/.auch.zshrc # for backward compatibility
...
if [[ ${AUCH_ZSH_NO_PLUGINS} != "1" ]]; then
    _dedup_zsh_plugins
    if [[ "$AUCH_ZSH_OMZ_DEFER" == "1" ]] && [[ -r $ZSH/oh-my-zsh.sh ]]; then
        # Loads the deferred oh-my-zsh plugin system by Auch // This is only for oh-my-zsh and compatibility
        _load_deferred_plugin_system_by_auch
