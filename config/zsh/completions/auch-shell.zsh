...
_auch_shell() {
    ...
    local built_in_commands auch_scripts wallbash_scripts
    ...
    if (( $+commands[hyde-shell] )); then
        ...
        auch_scripts=(${scripts_raw[@]%.*})  # Remove extensions
    ...
    if [[ $CURRENT -eq 2 ]]; then
        ...
        local all_commands=($built_in_commands $auch_scripts)
...
compdef _auch_shell hyde-shell